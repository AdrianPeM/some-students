        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden">
                <div class="flex flex-col p-6 sm:px-12 bg-white border border-gray-lightest rounded-lg">
                    <form id="complementaryActivity-form-update{{$complementaryActivity->id}}" method="POST" 
                        ...
                        <div class="grid grid-cols-2 gap-6 py-3">
                            ...
                            <div class="col-span-1">
                                ...
                                <x-select name="period">
                                    <x-slot name="content">
                                        <option value="Enero - Junio" name="period" {{old('period', $complementaryActivity->period) == 'Enero-Junio' ? 'selected':''}} >Enero - Junio</option>
                                        <option value="Agosto - Diciembre" name="period" {{old('period', $complementaryActivity->period) == 'Agosto-Diciembre' ? 'selected':''}} >Agosto - Diciembre</option>
                                    </x-slot>
                                </x-select>
                                @error('period')
                                    <p class="text-red">{{$message}}</p>
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="date" name="revision_date" id="revision_date" value="{{ old('revision_date', $complementaryActivity->revision_date) }}" class="mt-1 focus:ring-indigo focus:border-indigo block w-full shadow-sm sm:text-sm border-gray-light rounded-md">
                                @error('revision_date')
                                    <p class="text-red">{{$message}}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-span-1">
                                ...
                                <input type="number" name="hours" id="hours" value="{{ old('hours', $complementaryActivity->hours) }}" class="mt-1 focus:ring-indigo focus:border-indigo block w-full shadow-sm sm:border-gray-light rounded-md">
                                @error('hours')
                                    <p class="text-red">{{$message}}</p>
                                @enderror
                            </div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                        ...
                        <x-button class="ml-2" data-type="submit-form-btn" 
                            data-target-form="complementaryActivity-form-update{{$complementaryActivity->id}}">Guardar</x-button>
                    </div>
                </div>
            </div>
        </div>
